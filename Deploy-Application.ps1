...
Try {
	...
	[string]$appScriptDate = '02/07/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Copy-File -Path "$dirFiles\Greenshot.ini" -Destination "$envProgramFiles\Greenshot"

		$exitCode = Execute-Process -Path "$dirFiles\greenshot.exe" -Parameters '/VERYSILENT /CLOSEAPPLICATIONS' -WindowStyle 'Hidden' -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Stop-Process -name Greenshot

		$exitCode = Execute-Process -Path "$envProgramFiles\Greenshot\unins000.exe" -Parameters '/VERYSILENT /CLOSEAPPLICATIONS' -WindowStyle 'Hidden' -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }

		Remove-File -Path "$envProgramFiles\Greenshot" -Recurse
